...
Template variables (vd {current_date}, {current_year}) dùng str.format(**vars).

Usage (thay _get_agent_system_prompt trong agent.py):
    from prompt_loader import load_prompt
    prompt = load_prompt("agent_system", current_date=..., current_year=...)

Quy ước file:
    backend/prompts/agent_system.v1.txt          <- nội dung
    backend/prompts/agent_system.v1.meta.yaml    <- (optional) changelog/author/eval_run_id
"""
