...
switch ($Action) {
    ...
    "app" {
        ...
        Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location backend/src; if (Test-Path '../../.venv') { ..\..\.venv\Scripts\Activate.ps1 }; celery -A tasks.celery_app worker --loglevel=info -P solo" -Title "Celery Worker"
        Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location backend/src; if (Test-Path '../../.venv') { ..\..\.venv\Scripts\Activate.ps1 }; uvicorn app:app --host 0.0.0.0 --port 8002" -Title "FastAPI Backend"
        Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location frontend; if (Test-Path '../.venv') { ..\.venv\Scripts\Activate.ps1 }; streamlit run chat_interface.py --server.port 8501" -Title "Streamlit UI"
    }
    default {
